    ...
    if BATCHID.startswith("Exome") or BATCHID.startswith("GMPFocus"):
        ...
        qc_exec_stats = [
            QCExecStats(
                ...
                created='2024-10-18T11:45:26.826823+10:30', modified='2024-10-18T11:45:26.826844+10:30',
                is_valid=True,
                deduplicated_reads=int(float(exec_stats_dict.get('DedupReads', 0))),
                ...
                number_indels=int(float(exec_stats_dict.get('Indels', 0))),
                number_snps=0,
                #percent_10x_goi=,
                #percent_20x_goi=100.0,
                #percent_20x_kit=98.35,
                percent_250x_kit=float(exec_stats_dict.get('%bp250x_Kit', 0)),
                percent_error_rate=float(exec_stats_dict.get('%ErrorRate', 0)),
                percent_map_to_diff_chr=float(exec_stats_dict.get('%map2diff_chr', 0)),
                percent_read_enrichment=float(exec_stats_dict.get('%ReadEnrich', 0)),
                percent_reads=float(exec_stats_dict.get('%Reads', 0)),
                percent_softclip=float(exec_stats_dict.get('%SoftClipRate', 0)),
                percent_duplication=float(exec_stats_dict.get('%Duplication', 0)),
                reads=int(float(exec_stats_dict.get('Reads', 0))),
                sample_id_lod=float(exec_stats_dict.get('SampleID_LOD', 0)),
                sex_match=str(exec_stats_dict.get('SexMatch')),
                snp_dbsnp_percent=0,
                ts_to_tv_ratio=float(exec_stats_dict.get('Ts/Tv', 0)),
                uniformity_of_coverage=float(exec_stats_dict.get('%UniformCov', 0))
